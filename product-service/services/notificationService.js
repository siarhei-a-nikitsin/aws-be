...
export const sendNewProductsNotification = async newProducts => {
  ...
  const sendNotification = async (products, isExpensiveProducts) => {
    try {
      const result = await sns.publish({
        Subject: `New ${isExpensiveProducts ? 'expensive' : ''} products are created!`,
  ...
  expensiveNewProducts.length > 0 && await sendNotification(expensiveNewProducts, true);
  nonExpensiveNewProducts.length > 0 && await sendNotification(nonExpensiveNewProducts, false);
};